function UI(){
  ...
  this.mirr_apply = createButton('Apply').mousePressed(setMirroring).mousePressed(apply_pairing)
  this.mirr_cancel= createButton('Cancel').mousePressed(setMirroring).mousePressed(cancel_pairing)
  ...
  this.size_1 = createButton('1').mousePressed(setGearSize)
  ...
  this.size_1_R = createButton('1').mousePressed(setGearSize_R)
  ...
  this.left = createButton('←').mousePressed(moveCenter_left)
  this.right = createButton('→').mousePressed(moveCenter_right)
  this.center = createButton('◆').mousePressed(moveCenter)
  ...
  function apply_pairing(){
    if(pageMode == 1){pair_petal = 1}
    else if(pageMode == 3){pair_wing = 1}
  }
  function cancel_pairing(){
    if(pageMode == 1){pair_petal = 0}
...
//        highlightGearSize_R(stdSliderValue.wings.gearSize_R)
  ...
  function setGearSize(){
    gearSize = parseInt(this.elt.innerHTML)
    highlightGearSize(gearSize)
    _this.currentGearSize = parseInt(this.elt.innerHTML)

    if(pageMode == 1){
      gearSize_petal = gearSize
    }else if(pageMode == 3){
      gearSize_wing = gearSize
    }
    console.log("gear size = "+gearSize)
  }

  function setGearSize_R(){
    gearSize_R = parseInt(this.elt.innerHTML)
    highlightGearSize_R(gearSize_R)
    _this.currentGearSize_R = parseInt(this.elt.innerHTML)
    if(pageMode == 3){
      gearSize_wing_R = gearSize_R
    }
  ...
  function highlightGearSize_R(gearSize_R){
    if(gearSize_R == 1){
       _this.size_1_R.style("background-color",blue)
       _this.size_2_R.style("background-color",white)
       _this.size_3_R.style("background-color",white)
       _this.size_4_R.style("background-color",white)
     }else if(gearSize_R == 2){
       _this.size_1_R.style("background-color",white)
       _this.size_2_R.style("background-color",blue)
       _this.size_3_R.style("background-color",white)
       _this.size_4_R.style("background-color",white)
     }else if(gearSize_R == 3){
       _this.size_1_R.style("background-color",white)
       _this.size_2_R.style("background-color",white)
       _this.size_3_R.style("background-color",blue)
       _this.size_4_R.style("background-color",white)
     }else if(gearSize_R == 4){
       _this.size_1_R.style("background-color",white)
       _this.size_2_R.style("background-color",white)
       _this.size_3_R.style("background-color",white)
       _this.size_4_R.style("background-color",blue)
     }
  }
  ...
  function setServoAngle(){
    if(this.elt.innerHTML == "Continuous"){
      ...
      if(pageMode == 1){motorType_petal = 180}
      else if(pageMode == 3){motorType_wing = 180}
  ...
  this.button_OpenClose = function(){
    ...
    if(_this.currentServoAngle == 180){ // 180 servo
      if(_this.currentPairing == 0){ // cancel pairing
        _this.mirr_apply.show().position(138,415).style("background-color",white)
        _this.mirr_cancel.show().position(190,415).style("background-color",blue)

      }else if(_this.currentPairing == 1){  // paired!
        _this.mirr_apply.show().position(138,415).style("background-color",blue)
        _this.mirr_cancel.show().position(190,415).style("background-color",white)
      }

      text("Add Pairing :", 20, 430)

    }else if(_this.currentServoAngle == 360){  // 360 servo
      _this.mirr_apply.hide()
      _this.mirr_cancel.hide()
      _this.currentPairing = 0
    }

    _this.size_1.show().position(115,325)
    _this.size_2.show().position(150,325)
    _this.size_3.show().position(185,325)
    _this.size_4.show().position(220,325)

    _this.mtr180.show().position(50, 380)
    _this.mtr360.show().position(140, 380)
...
//  _this.Btn_plt.show().size(150,20).position(60,520)
  ...
  this.putText_OpenClose = function(){
    ...
    text("E", 25, 300)
    text("F", 145, 300)
    text("Gear Size :", 20, 340)
    text("Motor Rotation Angle :", 20, 370)
...
/*   from here:   slider section */
  ...
  /* from here: range of slider values */
